...
def query_to_search(keys: list[str], chart_type: str):
    ...
    if chart_type.lower() == "bar":
        ...
        graph = px.scatter(df.iloc[:numResults], x ="publicationDate", y="influentialCitationCount", size=np.power(df.iloc[:numResults]["citationCount"], np.full(numResults, 0.7)) + 5, hover_name="title", log_y=True, color="venue", title=f"Visualising the Academic Influence of Papers related to {query.capitalize()}" , width=1500)
